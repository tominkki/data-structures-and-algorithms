...
void Life::loop(void) {
  while (true) {
    this->update();
    this->print();
    std::this_thread::sleep_until(std::chrono::system_clock::now() + std::chrono::seconds(this->_delay));
  }
}
...
void Life::save_state(void) {
  std::cout << "Do you want to save current game state to file? ";

  if (!utils::user_says_yes()) { return; }

  std::string fname, comment;
  std::cout << "Enter filename: ";
  std::cin >> fname;
  std::cout << "Enter description for save file: ";
  std::cin >> comment;
...
void Life::initialize(void) {
  ...
  std::cout << "Set delay for animation: ";
  std::cin >> this->_delay;
}
